...
async def section_chosen(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await _apply_section(callback.message, state, subject, flow, target, key)
...
async def _apply_section(
    ...
    """Записывает выбранный раздел и ведёт дальше по тому потоку, из которого пришли."""
    telegram_id = message.chat.id
    label = _section_label(telegram_id, subject, key)

    # Клавиатуры старых сообщений живут вечно, и по ним нажимают уже после
    # того, как файл разложен. Делать вид, что что-то произошло, нельзя.
    if flow != FLOW_START and not target:
        await message.answer(
